...
feat_names = [
    'ttr', 'punct', 'pro1_tr', 'pro2_tr', 'pro3_tr', 'pron_tr', 'prop_of_N',
    'prop_of_adj', 'prop_of_prep', 'prop_of_v', 'prop_of_det', 'prop_of_ex',
    'prop_of_for', 'prop_of_to', 'prop_of_uh', 'genre'
    ]
...
with open('mc_features.csv', 'w') as out_file:
    for f in glob.glob(MC_DIR + '*.txt'):
        ...
        print(
            ttr(tok_text), punct_tr(tok_text), pro1_tr(tok_text), 
            pro2_tr(tok_text), pro3_tr(tok_text), pron_tr(tok_text), 
            prop_of_N(tagged), prop_of_adj(tagged), prop_of_prep(tagged), 
            prop_of_v(tagged), prop_of_det(tagged), prop_of_ex(tagged), 
            prop_of_for(tagged), prop_of_to(tagged), prop_of_uh(tagged), 
            subcorp(f),
            sep=',', file=out_file)
