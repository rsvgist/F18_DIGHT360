...
# MC_DIR = 'D:\\python_projects\\F18_DIGHT360\\assignments\\lyrics\\'
# MC_DIR = 'D:\\python_projects\\F18_DIGHT360\\assignments\\final_grammy_noms_clean.txt'
MC_DIR = 'D:\\python_projects\\F18_DIGHT360\\assignments\\final_grammynoms_61_clean.txt'
...
with open('mc_feat_names_grammys.txt', 'w') as name_file:
    name_file.write('\t'.join(feat_names))

with open('mc_features_grammys.csv', 'w') as out_file:
    for f in glob.glob(MC_DIR):
        print('.', end='', flush=True)  # show progress; print 1 dot per file
        with open(f, encoding= 'utf8' ) as the_file:
            # raw_text = clean(the_file)
            tok_text = nltk.word_tokenize(the_file)
...
###############################################################################
# Load dataset
with open('mc_feat_names_grammys.txt') as name_file:
    names = name_file.read().strip().split('\t')
len_names = len(names)
with open('mc_features_grammys.csv') as mc_file:
